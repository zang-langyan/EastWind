...
void main()
{
  ...
  light.radiance = vec3(0.1, 0.2, 0.2);
  ...
  diffuseColor += 5. * diffDot * light.radiance;
  float ambientFactor = 0.8f;
  ambientColor += (light.ambient.xyz * material.ka);

  float spec;
  if (diffDot>0){
      vec3 viewDir = vec3(normalize(v_CamPos - v_WordPos));
  ...
  float specValue = 0.2;
  specularColor += specStrength * spec * light.radiance;
