...
public:
      ...
      // r(r),g(g),b(b)
  {
    ...
    /* TODO: FIX THIS to join before Window closed */
    // for (auto& scene : EastWind::SceneManager::instance().GetScenes()) {
    //   for (auto& mesh : scene.GetMeshes()) {
    //     std::thread mesh_hit_test_thread = std::thread([&]() {
    //       while (true) {
    //         if (EastWind::Input::GetCursorRay().Hit(mesh)) {
    //           mesh.SetHit(true);
    //         } else {
    //           mesh.SetHit(false);
    //         }
    //       }
    //     });
    //     mesh_hit_test_thread.detach(); // Detach the thread to run independently
    //   }
    // }
  }
  ...
  void OnUpdate(EastWind::Timestep ts) override
  {
    ...
    // for (auto& scene : EastWind::SceneManager::instance().GetScenes()) {
    //   for (auto& mesh : scene.GetMeshes()) {
    //     /* TODO: Make the Hitting Task Running in another thread */
    //     // if (EastWind::Input::GetCursorRay().Hit(mesh)) {
    //     if (mesh.IsHit()) {
